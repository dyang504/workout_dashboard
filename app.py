...
## View
color_scheme={'white':"#F1EEF6","black":'#1E152A','green':'#8CFFBA','dark_grey':'#373E49','grey':'#9099AB','red':'#FF5A33'}
...
app.layout = html.Div(children=[
    html.H2(children='Overall Workout Trend',style={'color':color_scheme['green'],'paddingTop':20}),
    # generate_fact(df,"次",'次'),
    # generate_fact(df,"重量",'KG'),
    html.Div([
      html.Div(generate_table(table_df),className='pure-u-1-2'),
      html.Div(generate_bar_graph(total_by_sports_name),className='pure-u-1-2')
        ],className='pure-g'),
    html.Div(generate_graph(df),className='pure-u-1-2'),
     html.Div(generate_pie(),className='pure-u-1-2')
],style={'backgroundColor': color_scheme['black'],'margin':0,'hegith':'100%'})
